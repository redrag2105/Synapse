  ...
  Future<Either<Failure, List<JournalEntity>>> getTopJournalsByTopicId(
    ...
    try {
      final response = await _apiJournal.getJournals(
        // Filter: Tìm các nguồn (Sources) hosting bài báo thuộc chủ đề này
        filter: 'topics.id:$topicId',
        // Sort: Tạp chí có số lượng bài báo (works_count) nhiều nhất lên đầu
        sort: 'works_count:desc',
        perPage: limit,
        // Select: Tối ưu tải về
        select: 'id,display_name,works_count,cited_by_count,summary_stats',
      );

      final results = response['results'] as List;

      final journals = results.map((e) => JournalModel.fromJson(e)).toList();
